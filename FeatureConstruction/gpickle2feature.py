...
def main(args):
    ...
    # setting the path
    Path,OutputDir = normalizePath(args.input_path), normalizePath(args.output_path)
    ...
    for root, dirs, files in os.walk(Path):
        ...
        # setting the start index of files
        start_index = 0
        if os.path.exists(recordCsvPath):
            with open(recordCsvPath,'r') as f:
                lines = f.readlines()
                start_index = len(lines)-1
        else:
            WriteCSV(['filename','timecost'],recordCsvPath)
            
        files = files[start_index:]
        
        for index, f in enumerate(files):
            print(index+start_index, f)
